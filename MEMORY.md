...
- **交易周期系统** (2026-03-19 新增):
  - **目录结构**: `active/cycle-*/` 存放活跃周期，`archived/` 存放已归档周期
  - **周期文件**: `trade-suggestions.json` 记录交易建议
  - **报告路径**: `active/cycle-*/reports/` 存放本周期报告
  - **生命周期**: 上一周期结束 → 下一篇报告开启新周期 → 所有建议关闭后归档
  - **监控器**: `july-report-monitor` (PM2) 监控 `active/cycle-*/reports/` 并通知十四月
...
- **工作角色**:
  - 一月是她的上司，负责管理她
  - 七月是她的同事，她可以从七月的 `active/cycle-*/reports/` 文件夹获取日报
